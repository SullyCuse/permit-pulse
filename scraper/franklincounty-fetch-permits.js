...
async function fetchNewPermits(lastTimestampMs) {
  ...
  try {
    ...
    console.log('  [Franklin County] Submitting search...');
    await page.evaluate(() => { FormSupport.submitAction('BasicSearch'); });
    await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 30000 }).catch(() => {});

    console.log('  [Franklin County] Loading first results page...');
    await page.evaluate(() => {
    ...
    if (searchPageResponses.length === 0) {
      console.log('  [Franklin County] No results returned from SearchPage.');
      return { permits: [], maxTimestamp: lastTimestampMs || Date.now() };
    }
