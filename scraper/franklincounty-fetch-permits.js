...
// Franklin County, GA — SmartGov (Granicus) portal scraper.
...
// State key: franklincounty_last_timestamp

const BASE_HOST = 'https://co-franklincounty-ga.smartgovcommunity.com';
...
async function fetchNewPermits(lastTimestampMs) {
  ...
  console.log(`[Franklin County] Fetching permits since ${lastDateStr} (range: ${dateRange})...`);

  let puppeteer;
  try { puppeteer = require('puppeteer'); }
  catch {
    console.error('  [Franklin County] puppeteer not installed — skipping.');
  ...
  try {
    ...
    console.log('  [Franklin County] Loading search page...');
    await page.goto(SEARCH_URL, { waitUntil: 'networkidle2', timeout: 30000 });

    console.log(`  [Franklin County] Setting date range: ${dateRange}`);
    ...
    console.log('  [Franklin County] Clicking Search button...');
    ...
    console.log('  [Franklin County] Loading results via gotoPage(0)...');
    await page.evaluate(() => {
    ...
    if (searchPageResponses.length === 0) {
      console.log('  [Franklin County] No SearchPage response received.');
      return { permits: [], maxTimestamp: lastTimestampMs || Date.now() };
    }
    ...
    while (hasMore && currentPage < 50) {
      ...
      if (currentPage === 0) {
        if (rows.length === 0) { console.log('  [Franklin County] No results found.'); break; }
        if (headers.length > 0) {
          globalHeaders = headers;
          console.log('  [Franklin County] Columns:', headers.join(', '));
        }
      }

      if (rows.length === 0) { hasMore = false; break; }
      console.log(`  [Franklin County] Page ${currentPage + 1}: ${rows.length} rows`);
    ...
    console.error(`  ❌ [Franklin County] Error: ${err.message}`);
  } finally {
    await browser.close();
  }
  ...
  console.log(`[Franklin County] Found ${finalPermits.length} permits. Max timestamp: ${new Date(maxTimestamp).toISOString()}`);
