...
const BASE_URL = 'https://api-east.viewpointcloud.com/v2/sandyspringsga/records';
const PORTAL_BASE = 'https://sandyspringsga.portal.opengov.com/records';

// Permit type IDs to scrape from the ViewpointCloud portal
const RECORD_TYPES = [
  { id: 6552, name: 'Commercial Building Permit' },
  { id: 6426, name: 'Residential Building Permit' },
  { id: 6499, name: 'Mechanical Permit' },
  { id: 6498, name: 'Electrical Permit' },
  { id: 6500, name: 'Plumbing Permit' },
  { id: 6503, name: 'Land Disturbance Permit' },
  { id: 6504, name: 'Pool Permit' },
  { id: 6505, name: 'Retaining Wall Permit' },
  { id: 6506, name: 'Sign Permit' },
  { id: 6550, name: 'Demolition Permit' },
  { id: 6436, name: 'Right of Way Utility Permit' },
  { id: 6471, name: 'Tree Removal Permit' },
];

const HEADERS = {
  'Accept': 'application/json',
  'Origin': 'https://sandyspringsga.portal.opengov.com',
  'Referer': 'https://sandyspringsga.portal.opengov.com/',
...
async function fetchNewPermits(lastTimestampMs = 0) {
  const since = lastTimestampMs
    ...
    : 'beginning';
  console.log(`[Sandy Springs] Fetching permits since ${since}...`);
  ...
  // Filter to only permits submitted after our last timestamp
  const newRecords = unique.filter(r => {
    const ms = parseVpcDate(r.attributes?.dateSubmitted);
    return ms > lastTimestampMs;
  });

  console.log(`[Sandy Springs] ${unique.length} total active, ${newRecords.length} new since ${since}`);
