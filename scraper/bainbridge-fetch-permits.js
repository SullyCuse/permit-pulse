...
// City of Bainbridge, GA — SmartGov (Granicus) portal scraper.
...
// State key: bainbridge_last_timestamp

const BASE_HOST = 'https://ci-bainbridge-ga.smartgovcommunity.com';
...
async function fetchNewPermits(lastTimestampMs) {
  ...
  console.log(`[Bainbridge] Fetching permits since ${lastDateStr} (range: ${dateRange})...`);

  let puppeteer;
  try { puppeteer = require('puppeteer'); }
  catch {
    console.error('  [Bainbridge] puppeteer not installed — skipping.');
  ...
  try {
    ...
    console.log('  [Bainbridge] Loading search page...');
    await page.goto(SEARCH_URL, { waitUntil: 'networkidle2', timeout: 30000 });

    console.log(`  [Bainbridge] Setting date range: ${dateRange}`);
    ...
    console.log('  [Bainbridge] Clicking Search button...');
    const searchBtn = await page.$('button.sgc-button-primary, button[onclick*="Search"]');
    if (!searchBtn) {
      console.log('  [Bainbridge] Search button not found — skipping.');
      return { permits: [], maxTimestamp: lastTimestampMs || Date.now() };
    }
    await Promise.all([
    ...
    while (hasMore && currentPage < 50) {
      ...
      if (currentPage === 0) {
        ...
        if (headers.length > 0) {
          ...
          console.log('  [Bainbridge] Columns:', headers.join(', '));
        }
      }

      if (rows.length === 0) { hasMore = false; break; }
      console.log(`  [Bainbridge] Page ${currentPage + 1}: ${rows.length} rows`);
    ...
    console.error(`  ❌ [Bainbridge] Error: ${err.message}`);
  } finally {
    await browser.close();
  }
  ...
  console.log(`[Bainbridge] Found ${finalPermits.length} permits. Max timestamp: ${new Date(maxTimestamp).toISOString()}`);
