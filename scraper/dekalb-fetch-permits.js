...
const BASE_URL = 'https://dcgis.dekalbcountyga.gov/mapping/rest/services/Building_Permit_Applications/FeatureServer/0';
const PAGE_SIZE = 2000;

const OUT_FIELDS = 'applicationNumber,locationLine1,workType,WorkTypeDescription,status,issuedDateTime,primaryContactName,primaryContactType';

// SSL cert on dcgis.dekalbcountyga.gov is expired — bypass verification
...
async function fetchNewPermits(lastTimestampMs = 0) {
  const since = lastTimestampMs
    ...
    : 'beginning';
  console.log(`[DeKalb County] Fetching permits since ${since}...`);
  ...
  while (true) {
    const params = new URLSearchParams({
      where: `issuedDateTime > ${msToArcgisTimestamp(lastTimestampMs)}`,
      outFields: OUT_FIELDS,
      returnGeometry: 'true',
      outSR: '4326',
      orderByFields: 'issuedDateTime ASC',
      resultOffset: String(offset),
      resultRecordCount: String(PAGE_SIZE),
      f: 'json',
    });

    const { data } = await axios.get(`${BASE_URL}/query?${params}`, {
      timeout: 30000,
      httpsAgent,
    });
  ...
  console.log(`[DeKalb County] Found ${allFeatures.length} permits`);
  ...
  const permits = allFeatures.map(f => {
    ...
    return {
      permit_number:   String(a.applicationNumber),
      address:         a.locationLine1 ?? null,
      zip_code:        null,
      permit_type:     a.WorkTypeDescription || a.workType || null,
      description:     null,
      date_filed:      msToIsoDate(a.issuedDateTime),
      contractor_name: a.primaryContactName ?? null,
      applicant_name:  null,
